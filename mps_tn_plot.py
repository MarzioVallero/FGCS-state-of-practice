...
df = pd.read_pickle("data/approx_tn_data.pkl")
...
df['error'] = abs(df['approx_amplitude'] - df['exact_amplitude'])
...
g = sns.catplot(
    data=df, x="num_qubits", y="error", hue="rel_cutoff", col="circuit",
    capsize=.2, palette="bright", errorbar="se",
    kind="point", height=6, aspect=.75, log_scale=True
)
g.despine(left=True)

g.savefig('plots/pointplot_approx.pdf')
...
g.savefig('plots/violin_approx.pdf')
...
f.savefig('plots/distr_approx.pdf')
